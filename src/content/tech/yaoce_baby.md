...
**TL;DR**：
本博客起初是想随便写一些教程，给个大概的学习方向，但在整理我过往收藏的资料和学习的视频发现，视觉学习多而杂，特别是进入Ubuntu后的学习，需要了解和学习的东西很多，因此考虑到后面的发展给将来的新生以及现在大一同学做一个全面的知识体系是非常有必要的。最近我看到他们刚安装好了虚拟机并且学习linux的一些命令，回想我安装好Ubuntu学习的时候，完全是摸着石头过河，东看一些西看一些，也是比较迷茫，不知道要学什么,非常的有感触。于是就有了这个我熬夜吐血整理的简单学习入门教程，里面记录视觉可以学习的入门内容，不是每个都要搞懂，主要是搭建学习框架，不至于不知道往哪个方向，里面有表述不清楚或者错误的地方，欢迎批评斧正。
...
- 📖 [科学上网](#科学上网)：获取国外技术资料的必备技能，如稳定访问github
...
现在在诸如 Cursor、Claude Code、Trae 这样的 AI 代码编辑器兴起之后，一个新的概念开始出现，也就是氛围编程。作为氛围编程，就是给AI写代码，我们在旁边加油打气，等待他写好，不合适再让他改，体验一种氛围。哈哈哈，开玩笑。氛围编程是使用文字的提示让模型去替你生成程序，但是其背后是程序的语言本身只是外在，但背后的思想是统一的。差不多这个意思吧。
...
可能有些同学对AI嗤之以鼻，不相信他写的东西，比较抵制或者是恐惧，毕竟AI编程像黑盒子，无法看清。对于AI编程，在我看来如今是一种趋势，AI现在已经非常强大，但是你能力的上限就决定了AI的上限，总的来说，AI是一种工具，它放大了你的能力，你可以作为一个架构师去架构你的项目，它也可以是老师，不会不懂的问题向她请教，记录总结下来很有必要，因此做笔记是非常重要的。这里关于如何使用AI，我不再过多赘述，仅仅表达我的看法，仁者见仁，智者见智吧。
...
对于这些视觉模块，不能只是跑官方的例程，想要学好，要去深入看官方封装好的库函数，了解常用库函数的内部结构原理，看懂，能知道怎么调用调试，以便根据题目跑出最大性能，因此能把这些视觉模块弄透也是非常好，非常厉害的，不必过度追求高性能开发板。
...
想要安装Ubuntu，可以参阅这篇教程：[Ubuntu/Windows双系统的安装](https://blog.csdn.net/NeoZng/article/details/122779035)，当然，学习时使用虚拟机也是不错的选择，这能给你更大的试错空间，不用担心把系统搞奔溃。---建议多方收集资料在开始，比如去B站找一些资料看看流程，明确了步骤再开始安装，装不好电脑就会出问题到时候就麻烦了，建议有电脑基础再安装。
...
我们实验室开始的时候都是用u盘拷贝程序，或者QQ打包压缩发送，效率太慢了，多了如果没有命名好版本，还容易搞混。git来实现版本控制非常的方便，如果代码改动太多，改不回来，还可以回退到之前保存的版本，建议实验室的成员都来使用，无论是日常学习项目还是比赛时都用得上。
...
GitHub 是全球最大的开源社区，里面有大量优质项目、模板和学习资源，推荐大家多多使用和查找，会有意想不到的效果。
...
4.前3个是对于控制题小车，这里控制题飞机，用于要上激光雷达，最低也得要树莓派，能再往上换个更好的最好，本篇更多知识点，包括ROS2,导航等等，是根据飞机题用到的知识点来写的。
