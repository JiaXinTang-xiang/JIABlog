...
知识库节点的创建、移动、查询属于 `wiki` 模块，正文编辑属于 `docs` 模块。一个比较适合长期技术笔记的结构是：
...
我更倾向于把“视觉学习”写成一篇持续生长的长文，再按知识模块逐章补齐：结构完整、链接不会到处散，同时每一章仍能单独校对和修改。先通过下面的帮助确认当前版本支持的知识库子命令：
...
没有独立窗口。打开 PowerShell 或 Windows Terminal，输入：

```powershell
lark-cli --help
```

这就是打开并使用 CLI。要打开知识库页面，则直接在浏览器打开知识库 URL；CLI 负责读取和修改飞书里的资源。

### `auth status` 成功，但业务命令失败

先看报错中的 `missing_scopes`。用户身份用对应 `--scope` 增量授权；机器人身份去飞书开放平台补应用权限。不要为了图省事一次申请不必要的所有权限。

### 授权页面提示过期

设备授权链接是临时的，重新运行登录命令生成新链接即可：
